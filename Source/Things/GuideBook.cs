...
namespace RimWriter
{
    public class GuideBook : ThingBook
    {
        ...
        public override void ReadWarning(Pawn p)
        {
            base.ReadWarning(p);
            if (p?.skills?.GetSkill(skillDef) == null)
                Messages.Message("RimWriter_WarningNoSkill".Translate(p, skillDef.LabelCap), MessageTypeDefOf.CautionInput);
            if (p?.skills?.GetSkill(skillDef)?.TotallyDisabled == true)
                Messages.Message("RimWriter_WarningSkillDisabled".Translate(p, skillDef.LabelCap), MessageTypeDefOf.CautionInput);
        ...
        public string Author
        {
            get
            {
                if (author == null)
                {
                    ...
                    if (authorNameInt != null && authorNameInt != TaggedString.Empty && !authorNameInt.ToString().StartsWith(" "))
                    {
                    ...
                    else
                    {
                        ...
                        Traverse.Create(CompArt).Field("authorNameInt").SetValue(new TaggedString(author));
                    }
                }
                return author;
            }
